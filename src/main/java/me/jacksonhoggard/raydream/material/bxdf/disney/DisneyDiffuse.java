...
public class DisneyDiffuse extends BRDF {
    ...
    public Vector3D eval(Vector3D wo, Vector3D wi) {
        ...
        double fdV = MathUtils.lerp(1.0, fd90, pow5(1.0 - Math.abs(wo.dot(ns))));
        double fdL = MathUtils.lerp(1.0, fd90, pow5(1.0 - Math.abs(wi.dot(ns))));
        double fd = fdV * fdL;
        ...
        double fssV = MathUtils.lerp(1.0, fss90, pow5(1.0 - Math.abs(wo.dot(ns))));
        double fssL = MathUtils.lerp(1.0, fss90, pow5(1.0 - Math.abs(wi.dot(ns))));
